...
void BudgetManager::ProcessQuery(std::unique_ptr<Query> query) {
    if (query->GetOperationName() == "Earn") {
        ...
        for (size_t i = first_day_index; i < first_day_index + days; ++i) {
            budget_[i].second.income_ += income_per_day;
        }
        ...
        for (size_t i = first_day_index; i < first_day_index + days; ++i) {
            income_summ += budget_[i].second.income_ - budget_[i].second.spend_;
        }
        ...
        for (size_t i = first_day_index; i < first_day_index + days; ++i) {
            if (budget_[i].second.income_ > 0) {
                budget_[i].second.income_ *= (1.0 - (payTaxQuery->GetTax()/100.0));
            }
        }
        ...
        for (size_t i = first_day_index; i < first_day_index + days; ++i) {
            budget_[i].second.spend_ += spend_per_day;
        }
    }
}
